...
func decodeArtifactConfig(block *hcl.Block) (*Artifact, hcl.Diagnostics) {
	artifact := &Artifact{
		Type:      block.Labels[0],
		Name:      block.Labels[1],
	...
	if !hclsyntax.ValidIdentifier(artifact.Type) {
		diags = append(diags, &hcl.Diagnostic{
			Severity: hcl.DiagError,
			Summary:  "Invalid builder type",
			Detail:   "A builder type must start with a letter and may contain only letters, digits, underscores, and dashes.",
			Subject:  &block.LabelRanges[0],
		})
	}
	if !hclsyntax.ValidIdentifier(artifact.Name) {
		diags = append(diags, &hcl.Diagnostic{
			Severity: hcl.DiagError,
			Summary:  "Invalid artifact name",
			Detail:   "An artifact name must start with a letter and may contain only letters, digits, underscores, and dashes.",
			Subject:  &block.LabelRanges[1],
		})
	}

	artifact.Config = b.Config
